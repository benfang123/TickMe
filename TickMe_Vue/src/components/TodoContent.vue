<template>
  <div class="todo-content">
    ...
    <div v-else class="null">
      <!-- 如果vux中的theme是这个数据的颜色就显示这个图片，这个颜色不是css来改变的 -->
      ...
      <svg-icon icon-class="null" v-if="$store.state.theme == 'darkly'"></svg-icon>
      <span
        style="
          color: var(--Gray-6);
          font-size: 1rem;
          ...
          letter-spacing: 1px;
        "
        >当前暂无待办，点击<span style="color: var(--Theme-5)">
          右上方按钮 </span
        >添加</span
      >
    </div>
...
export default {
  ...
  mounted(){
    console.log(this.visibleListsAll)
  },
  computed: {
    // 用于展示的待办数组
    visibleListsAll() {
