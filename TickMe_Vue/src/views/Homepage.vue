<template>
	<div ref="mainHome" class="Main-bottom-plate" :style='{filter:"brightness("+$store.getters.luminance+")"}'>
		<div id="app" >
		  <!-- 左侧导航栏 -->
		  <div id="tab"  >
		    <!-- Logo -->
		    <div class="logo-box">
			  ...
			  <p class="logo-name">TickMe</p>
			  <!-- <svg-icon icon-class="Logo" id="logo-text"></svg-icon> -->
		    </div>
		    <!-- 用户信息 -->
		    <div class="user" @click="openPersonalCenter(true)">
		      <!-- 头像 -->
				  <img class="profile" :src="`http://localhost:3000/images/${this.$store.state.usersList.name}-header.jpg`" alt="用户头像">
		      <!-- 用户名 -->
		      <p class="username">昵称:{{this.$store.state.usersList.screenName}}</p>
		    </div>
		    <!-- 路由按钮 -->
		    <router-link
			...
			<el-dialog :visible.sync="dialogTableVisible" width="450px" height="700px" center :modal-append-to-body='false'>
		  ...
		  <div class="main">
		    ...
		    <router-view v-on:hideSearchBar="handleHideSearchBar" ></router-view>
		  </div>
		</div>
	</div>
...
<script>
	import PersonalCenter from "@/components/PersonalDetails.vue"
export default {
	components:{PersonalCenter},
  data() {
    return {
		showSearchBar: true,//判断需不需要显示搜索框；例如我的聊天室我就不想显示这个
		dialogTableVisible:false,//判断是否显示个人中心，默认不显示
      selectPath: "/homepage",
      routers: [
        {
          class: "my-todo",
      ...
      declaration: document.documentElement.style,
    };
  },
  watch: {
    // 侦听路由选中
    ...
    '$store.state.theme':function(newVal){
      if(newVal=='darkly'){
        this.$refs.mainHome.style.filter='invert(1) hue-rotate(180deg)'
      }else{
        this.$refs.mainHome.style.filter=''
      }
    }
  },
  methods: {
    changeThemes(theme) {
	 ...
	 handleHideSearchBar(hide) {
	      this.showSearchBar = !hide
	    },
	openPersonalCenter(status){
...
/* 弹窗遮罩的样式 */
.my-dialog-class__wrapper {
  background-color: rgba(0, 0, 0, 0.5);
}

/* 弹窗内容区域的样式 */
.my-dialog-class__body {
  width: 400px;
  height: 600px;
}
